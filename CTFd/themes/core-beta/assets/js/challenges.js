...
Alpine.data("Challenge", () => ({
  ...
  async submitChallenge() {
    ...
    // Highlight incorrect submission
    if (this.response.data.status === "incorrect") {
      // Find the challenge in the ChallengeBoard and set incorrect=true
      const board = Alpine.store("challengeBoard");
      if (board && board.challenges) {
        const chal = board.challenges.find(c => c.id === this.id);
        if (chal) {
          chal.incorrect = true;
          setTimeout(() => { chal.incorrect = false; }, 1000);
        }
      }
    }
...
Alpine.data("ChallengeBoard", () => ({
  ...
  getSolvedCount() {
    return this.challenges.filter(c => c.solved_by_me).length;
  },
